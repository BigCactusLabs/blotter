...
def expected_outputs():
    ...
    # Both fixtures start with the same duplicate sentinel, so this timestamp
    # is intentionally shared by the add duplicate-detection measurement.
    if manifest["duplicate_add"]["now"] != build_fixture(10_000)["duplicate_add_now"]:
...
def main():
    ...
    parser.add_argument(
        ...
        help="directory for scale-1k.jsonl, scale-10k.jsonl, and deterministic metadata",
    )
    parser.add_argument(
        "--check",
        action="store_true",
