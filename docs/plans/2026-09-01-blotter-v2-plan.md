...
| Fact | Value |
|---|---|
| Crate / envelope contract | 1.0.0 / `meta.contract` 6 |
...
Routing: full chain. implementer-opus-med in a worktree (silent-failure domain: persistence, identity); pr-reviewer-xhigh on the diff plus `codex review` for the cross-model axis. `gate-5x.sh` required. The pre-implementation critique ran 2026-09-01 (§11) and landed as r50 before the implementer was briefed. Landed 2026-09-02 as PR #19 into `v2` (TASK-75 retired). Claude-only round by instruction: one pr-reviewer-xhigh pass (LAND; its probe-cost finding halved read time on 100k records) and a Codex GitHub-bot P2 on `doctor`'s record-kind join, both fixed before merge; no `codex review`. Two readings recorded for follow-up: the mixed cut/dogear rejection is unconditional and also fires under `--amend` (r48 states no condition; relax to `!amend` if wrong), and `verify.resolution.disposition` plus `digest.accepted_cuts` are Phase 5.
...
## 12. Progress review after Phase 3, 2026-09-02

A second progress review read the `v2` tree after TASK-74 and TASK-75 landed. Its overall verdict — keep moving, no architectural reversal, the code is validating the simplification thesis — needs no change. It kept two choices this plan had left open to challenge: the unconditional rejection of mixed cut/dogear resolve batches (including under `--amend`) and the refuse-and-move-aside migration posture. Both stand. Its four changes were checked against r48–r50 and the code, and land as amendment r51 plus backlog rulings:

- **ID widths (adopted, before Phase 4).** Cut 12 hex, dogear 20 hex, promotion planned at 16 hex under one namespace, with r48's ambiguity-before-kind rule and no exact-full-ID precedence, meant a complete cut ID could prefix a dogear ID and become unresolvable — r50 had recorded and accepted that. The widths were residue from when width separated namespaces; `bl2` plus the kind field does that now. r51 makes every v2 identity 10 digest bytes, `bl_` + 20 hex. New TASK-81, a dependency of TASK-76 so promotion never introduces a third width. The r50 48-bit birthday note is withdrawn with it.
- **TASK-79 into the 1.0 bar (adopted).** `verify` still partitions against the winning resolution's `ts`, so a note-only `--amend` moved the recurrence boundary without re-deciding anything. r51 moves the boundary to `disposition_ts` and exposes `disposition_ts` in `resolution{}`, because a recurrence can otherwise legitimately predate the displayed `resolution.ts`. TASK-79 is now a dependency of TASK-77 and lands in the Phase 5 PR.
- **`suggested_action: "graduate"` (adopted as removal, not rename).** The field was one constant from before `retrospect` and `promote` existed. r51 withdraws it from the `triage` and `digest.chronic` cluster shape. Phase 5, in TASK-77.
- **TASK-71 as a precision gate (adopted).** `retrospect` consumes triage's clustering at a threshold of two linked records, so a false rare-token link is input to the promotion layer, not a cosmetic defect. r51 makes the r44 ceiling's precision a measured Phase 5 gate: rerun the linked-pair measurement across several logs after the admission floor; reshape only if unrelated clusters are still material. TASK-71 is a dependency of TASK-77 with the five-log corpus recorded in its notes (all v1 logs, so the measurement runs on the 0.15 binary).
- **`main` self-orientation (adopted).** Phases 4–6 remain, so the merge is not imminent. `AGENTS.md` on `main` now carries an explicit unreleased-v2 paragraph (commit e2903c2) saying `main` compiles contract 5 while r48+ describe the `v2` target; the paragraph is removed by the `v2` → `main` merge.

Phase 4 proceeds once TASK-81 merges. Phase 5's scope is now r48 §verify/§retrospect/§digest plus the three r51 rulings.


## 13. Frontier corroboration review after Phase 4, 2026-09-02

`docs/research/2026-09-02-v2-frontier-corroboration.md` re-ran the frontier pass after Phase 4 and concluded the architecture needs no structural change before 1.0.0, recommending two design-language invariants and one wording rule and nothing in code. The note is opinion, so before folding it in, every cited claim was checked against its live source and a counter-evidence hunt was run for the three positions the note takes without a dissenting source (Claude sonnet workers, read-only; reports in the session scratchpad). Verdict: adopt the three rules as amendment r52, demote one confidence claim, correct three citations in the note itself.

- **Sources (verified).** All twelve URLs resolve. Ten claims are supported verbatim, including every figure quoted from Jiang et al. (8,135 trials; 65.7% vs 4.5%; 29.6% → 3.3%). Three corrections: the arXiv page at 2602.01869 carries the title *Skill-Pro*, not *ProcMEM* (the ICML 2026 poster carries *ProcMEM*; same paper, renamed); Chen et al. never write "evidence promotion as a security boundary" — the paper says skill creation is a security boundary and, separately, that evidence promotion should be treated as "a provenance-sensitive authorization decision"; and the 29.6% → 3.3% precision figure is an average across two model pairings (16.9% → 0.7% and 42.3% → 5.9%), which the note does not say. The note now carries these in a verification section; none of them changes a ruling.
- **Promotion as a trust boundary (adopted, r52).** Phase 4 already shipped `promote` as an explicit mutation with pinned sources; the security preprints supply the reason that shape must not be relaxed. r52 forbids the automatic path by name: nothing in blotter calls `promote`. `AGENTS.md` gains the invariant.
- **Recurrence is not independence (adopted, r52).** Qi et al.'s correlated-trace framing is directly on point and is the sharper reason behind the r51 linkage gate. No score, no field.
- **`verify` negative-result wording (adopted, r52).** No envelope change; the `schema` wording lands with TASK-77, which now carries it as an implementation note.
- **Admission floor confidence (demoted).** The note rates selective admission "strongly corroborated". The counter-evidence hunt found one credible, dated primary dissent: Databricks (2026-04-10) reports raw conversation logs filtered only by an LLM judge rising from 2.5% to over 50% on their eval, past an expert-curated baseline at 33.0%. It does not argue for unfiltered retention — the judge selects — and it measures retrieval memory for task accuracy, not friction claims about a repository, so the floor's direction stands. But it is the evidence the §2 caveat anticipated, and r52 records it so the floor is not read as a settled effect. The other two counter-questions came back thin: nothing found showing gate-free promotion beats a human gate (the survey material found says the bottleneck is verification at admission, which is the gate), and no current source argues recurrence is a misleading triage signal.
