...
This closes every component shape the redactor can leave behind a generic home prefix. A component whose leading `~` stands directly ahead of surviving verbatim bytes that are neither `~` nor the secret marker is not reachable from the write side, by the same mirror that keeps the two scanners agreed. Wherever the stored bytes would let the scanner's generic prefix fire, the redactor's identical generic rule fired first on the raw text and consumed the whole component — `/Users/-Users-alice-foo` is the instructive case, storing `~` rather than `/Users/~-foo`, because a dash is not a slash-form component terminator — and wherever the generic start rule blocks the redactor, it blocks the scanner at the same position. What an exact home match can leave standing against its emitted marker is bounded by its end boundary (r23): a slash, the separator, a delimiter, or the end of input, each of which also ends the scanner's component, so the marker stands bare there; and the secret pass writes only its own marker (r25). Should a later pass produce a shape outside this enumeration, it fails safe — the gate flags a line holding no home bytes, the r30 class with the gate at fault — and the rule widens then, with the shape in hand, not speculatively now.
