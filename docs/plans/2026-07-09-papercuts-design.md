...
The rule lands on the redactor: after replacing a matched prefix with `~`, scanning resumes immediately after the match instead of at the token's end. Every boundary rule is unchanged — the tail is simply scanned under the same rules doctor already applies, so the two scanners once again encode one class by construction. The examples above now store as `~/x/~-y` and `~/backup~`, which doctor passes. The rest of a matched token still stores verbatim whenever it contains no home form of its own, so r23's foreign-slug limitation and r30's accepted `~-Documents-…` cwd spelling are unchanged, as is the slash-form rule that a nested generic path such as `~/Users/bob` (preceded by a non-delimiter) stays untouched and unflagged.
