...
This closes the marker-plus-marker shapes, not every byte the redactor can leave against a marker. A home match can emit `~` directly ahead of surviving verbatim bytes that are neither `~` nor the secret marker — `/Users//-Users-alice-foo` stores as `/Users/~-foo`, and an assignment or URL span leaves shapes such as `~-token=<redacted>` — and those components still report, on lines holding no home bytes. That residual is left open here for the same reason r39 left this one: accepting any leading `~` would reverse r39's real-directory-name decision. It is tracked as TASK-61.
