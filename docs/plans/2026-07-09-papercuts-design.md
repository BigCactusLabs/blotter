...
One compatibility consequence: `doctor --leaks` is a CI gate (r22), so a log holding one of these two shapes and no other finding now passes at exit 0 where it previously failed at exit 1. No stored bytes change, and nothing that reported a leak on real home bytes stops reporting it.
