...
**Unreleased v2 (read this before self-orienting on `main`).** The design doc's amendments from r48 onward describe contract 6, which is being built on the `v2` branch and has not merged. The code on `main` implements contract 5 (amendments through r47): `severity`, `source`, `pc_`/`bl_` IDs at the r6 widths, the `hook` subcommand and the `auto` lane. When the newest amendment and the compiled code disagree here, the code on `main` is what ships from `main` and the amendment is the target `v2` is moving toward; do not "fix" `main` toward contract 6. The plan and its progress record are `docs/plans/2026-09-01-blotter-v2-plan.md`. This paragraph is removed by the `v2` → `main` merge.
