...
Nothing else under `docs/` is contract. `docs/superpowers/specs/` holds specs for behaviour in the **current** release, each carrying a `Status:` line — the directory is often absent, and that is normal: archiving the last spec empties it and git does not track an empty directory, so 0.14.0 and 0.15.0 both shipped with no spec file. Current-release behaviour is then the design doc's newest amendments plus `blotter schema`, not a missing page. `docs/archive/` holds everything superseded — shipped plans and specs whose release is no longer current. (The pre-fork remediation records were deleted in the TASK-33 open-sourcing scrub: they leaked private detail in prose, and provenance does not survive a privacy conflict.) `docs/research/` holds one-off research notes. Both directories and the v2 plan under `docs/plans/` are gitignored working notes since 2026-09-03: they exist on the maintainer's machine and not in a fresh clone, so a changelog or design-doc line that cites one of them is provenance, not a link to follow. Only the design doc, `docs/reference.md`, and any current-release spec are public. `docs/reference.md` is the human-facing reference the README links to — every command, flag, and record rule in prose; keep it current when a surface changes, and remember it is descriptive, not contract. When any of it disagrees with the design doc, the design doc wins.
...
When you hit friction while working here, log it in the moment — if it clears the admission floor. Blotter is a selective ledger, not a transcript. A cut is a claim that something in this repo got in the way and that knowing about it has future value.

```bash
cargo run -- add "what you were doing → what got in the way" --tag <area> --impact low|material|blocking
```
