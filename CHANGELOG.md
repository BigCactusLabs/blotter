...
- `blotter add -` and `blotter dogear -` no longer buffer unbounded stdin (TASK-31). An endless producer grew the allocation until it failed, which under `panic = "abort"` is an abort with no envelope. The raw read is capped at 1 MiB, matching `--stderr-file` and the hook payload. Oversized input that redacts below the 10000-byte text limit is still accepted, as r25 requires.

- `sweep --registry` reports a missing registry file as `not_found` (exit 66) instead of `io_error` (exit 74), matching `--file` and `--stderr-file`; permission failures still exit 77 (TASK-31).

- `export --format otlp-json` writes stdout through the shared writer, so a broken pipe or a failed write is reported as a structured error instead of being suppressed — completing TASK-31.2, which had left this one raw-stdout path out.

- A non-UTF-8 `BLOTTER_AGENT` is a `config_error` (exit 78) instead of being silently discarded and the record filed under a detected or default agent, matching how `BLOTTER_NOW` already handles the same case (TASK-31).

- `list` and `export` validate `--since` before opening the log, so an invalid value exits 2 in every command that accepts the flag; `list` also checks its `--severity`/`--kind` conflict first. This is r28's stated principle, which `digest`, `archive`, and `sweep` already followed (TASK-31).

- `hook exec` no longer appends a second line carrying an existing cut ID. Only reachable under a frozen clock, where a resolved command replayed at the same instant recomputes the same ID: the append is skipped and `BLOTTER_HOOK_EXPLAIN=1` names the reason. A resolved command filed at a later instant still refiles, as r8 and r25 intend.
