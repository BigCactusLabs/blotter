...
- Evidence redaction and `doctor --leaks` now end an exact-home match at the redaction marker `~`, and at a `-` that begins a dash-encoded home form, closing the end-boundary gap r40 left open (TASK-62). r40 removed the exact home's start boundary but its end boundary still rejected a match followed by any byte outside `/`, that spelling's own separator, or a delimiter — so with `$HOME=/Users/alice` the r40-era evidence `-Users-/Users/alice-Users-alice` stored as `-Users-/Users/alice~`, leaving the slash-form home standing verbatim against the marker that replaced its dash-form neighbor, and the gate passed real home bytes at exit 0. A `-` now ends a dash-encoded match unconditionally, because it is that spelling's own path separator; it ends a slash-form match only when a dash-encoded home form — the exact dash-encoded current home, or a generic `-Users-`/`-home-` prefix — begins there, because in a slash path a `-` is an ordinary name byte and `feature/Users/alice-backup` still holds no home path. `~` ends a match on both sides, because the marker is blotter's own output and the redactor must be able to match home bytes standing against one. With `$HOME=/Users/alice`: `-Users-/Users/alice-Users-alice` now stores `-Users-~~`; `-Users-/Users/alice-Users-bob-x` stores `-Users-~-Users-bob-x`; `x/Users/alice~y` stores `x~~y`; `feature/Users/alice-backup`, `https://host/Users/alice-old`, `x/Users/alice2`, `x/Users/alice.bak`, and `x-Users-alicexyz` still store verbatim and pass, exactly as today. `doctor --leaks` gains the matching scanner acceptance — a generic home prefix is not a leak when its component's leading `~` is immediately followed by a `-`, read on the decoded text a line that parses as JSON exposes (r43) — closing the one new marker composition this widening leaves in the logical text. Two costs, both already paid by the dash spelling before this change: a slash home genuinely followed by a dash-encoded path that is not itself a home over-redacts (`feature/Users/alice-Users-alice-fix` stores `feature~~-fix`), and the r25 ordering hazard widens by the same step — an entropy token containing one of these shapes can be split by the emitted `~` and fall below the entropy thresholds. Compatibility: redaction runs before the identity hash, so a raw input holding one of these shapes now computes a different record ID than it did before this change; stored history is never rewritten. `doctor --leaks` is a CI gate, so a log holding one of these shapes and no other finding now fails at exit 1 where it previously passed at exit 0 — the point of the change, because the r40-era shape is a real leak. **Upgrading:** a log that already stored an r40-era shape such as `-Users-/Users/alice~` now fails the gate with no `--fix` repair path (`--leaks` stays diagnose-only); the remedy is `archive` or a manual edit of the affected line. A residual under-redaction stays deliberately out of scope: a slash home followed by an ordinary name byte, `x/Users/alice.bak`, still stores home bytes verbatim and passes, the same choice r40 made for `-Users-alicexyz`. Design doc r42.

- `doctor --leaks` now scans a physical line that parses as JSON on its decoded text — every string, at every depth — instead of the raw encoded bytes, and keeps today's raw-byte scan only for a line that does not parse (TASK-63). The split is on parse success, not on record validity: any line JSON can parse is scanned decoded, whether or not it is a valid blotter record. The scanner previously read JSONL-encoded bytes while the redactor writes logical text, and the encoding is not injective over the byte classes the leak rules key on: with `$HOME=/Users/alice`, a line whose field decodes to `/Users/~-/Users/alice` — a real current-home path — can spell those slashes as JSON's optional `\/` escape, so the inner home never appears as literal `/` in the stored bytes and no raw scan reached it, while the accepted `~-` prefix suppressed the generic match that would otherwise have reported; blotter's own encoder never writes that escape, but a hand-written or foreign line may, every enumeration of encoder-escape sequences hid the same hole rather than closing it. Scanning the decoded text closes it as a class: an escaped home path is literal once decoded and the exact-home scan reports it at its own index, and the marker-composition acceptances (r39, r41, and r42's new `~-` member) are stated on that same decoded layer, where they mean exactly what they mean on the write side. A line that does not parse — including a torn or malformed one — keeps the unchanged raw-byte scan and its existing acceptances, so `doctor`'s coverage of malformed lines (r22) does not regress; the raw layer gains no new acceptance rule. Separately, the stderr excerpt's 4096-byte truncation now backtracks to the start of an emitted secret-marker span when, and only when, the cut would fall inside one, so the stored excerpt never splits `<redacted>` into a fragment such as `~<re`; the redact-then-truncate order (r25) is unchanged, and text that merely resembles a marker but was never written as one, `…~<red` at the truncation boundary, keeps its bytes. Together these close both TASK-63 raw-layer shapes: a marker directly against a JSON-escaped delimiter (now unreachable on any line blotter successfully wrote; still raw-flagged, failing safe, only on a line that does not parse) and a marker split by truncation (no longer produced). Compatibility: this is a CI gate and the restructuring flips it both ways — a valid line whose escaped bytes decode to a home path now fails at exit 1 where it passed at exit 0, the leak-catching direction, and a valid line carrying a raw-layer false positive, an encoder escape behind a marker, now passes at exit 0 where it failed at exit 1. Malformed lines report exactly as before. No stored bytes change on the scanner side and no record ID moves; the write-side backtrack moves stored bytes only for a `--stderr-file` excerpt cut inside an emitted marker, and moves no ID, because `compute_id` hashes never include evidence (r34). `--deny` is unchanged and still matches literals against raw bytes on every line. Design doc r43.

- Evidence redaction and `doctor --leaks` now recognize the exact current home in its dash-encoded spelling wherever its bytes appear, with no start boundary (TASK-60). r23 gave every dash-form match one start-boundary rule — token start, an evidence delimiter, or a preceding `/` — but only the generic `-Users-`/`-home-` prefixes needed it; applied to the user's own bytes it withheld a match. With `$HOME=/Users/alice`, the evidence `-Users--Users-alice-y` stored verbatim and passed the gate at exit 0, as did `x-Users-alice` and a home nested in an encoded path such as `-tmp-backup-Users-alice-y`. The exact slash-form home has never carried a start boundary and redacts all three equivalent slash spellings. These now store as `-Users-~-y`, `x~`, and `-tmp-backup~-y`. The end boundary is unchanged, so `-Users-alicexyz` is still not a match, and the generic dash prefixes keep r23's start-boundary rule exactly as written: `dir-Users-x` and `-Users--home-bob-x` stay unrewritten and unflagged on both sides. Two costs are accepted, both already paid by the slash spelling: a non-path token containing the home's dash spelling over-redacts (`feature-Users-alice-fix` stores as `feature~-fix`), and because home rewriting runs before the secret pass, the emitted `~` can split a high-entropy token that contains the home bytes so its fragments fall below the entropy thresholds. Two compatibility consequences: redaction runs before the identity hash, so a raw input holding one of these shapes now computes a different record ID than it did before this change, and because `doctor --leaks` is a CI gate, a log holding one of these shapes and no other finding now fails at exit 1 where it previously passed at exit 0. Nothing that reported a leak before stops reporting one, and stored history is never rewritten. One adjacent under-redaction of the end-boundary rule is out of scope and tracked as TASK-62. Design doc r40.

- `doctor --leaks` no longer reports a leak on the redaction marker composed with itself or with the secret marker behind a generic home prefix (TASK-59, the entropy-marker residual r39 left open). r39 accepted a component that is exactly `~` on the premise that the marker always stands alone, but r38 made home scanning resume immediately after a match, so two home forms can abut and the redactor writes `~~` — with `$HOME=/Users/alice`, `/Users//Users/alice/Users/alice` stores as `/Users/~~`, and under r40 `-Users--Users-alice-Users-alice-y` stores as `-Users-~~-y`. The secret pass runs after home rewriting over a token class that holds no `~`, so it swallows the separator standing behind an emitted marker: `/Users//Users/alice/<32-char high-entropy token>` stores as `/Users/~<redacted>`, and the compositions stack as `~~<redacted>`, `~<redacted>~`, and `~<redacted>@host`. Each is blotter's own write, holds no home bytes, and tripped blotter's own gate. A generic home prefix is now not a leak when its username component is the bare marker `~`, or when that leading `~` is immediately followed by another `~` or by the literal `<redacted>`, whatever follows after, in the slash and dash-encoded forms alike; a component whose second element is anything else, such as `/Users/~abc`, is a real directory name and still reports. Detection of the exact current home is unaffected — the scan matches at every position and under r40 the exact home carries no start boundary — so home bytes inside an accepted component's tail still report at their own index. Only the scanner changes: no stored bytes change and no record ID moves. Because `doctor --leaks` is a CI gate, a log holding one of these shapes and no other finding now passes at exit 0 where it previously failed at exit 1. In the logical text no narrower residual survives — the redactor's generic rule consumes any component its scanner twin could read, and an exact match's end boundary only ends against bytes that also end the component; two pre-existing raw-layer false positives remain (a JSON-escaped delimiter directly behind a marker, and the stderr excerpt truncating a marker), fail safe, and are tracked as TASK-63. Design doc r41.

- `doctor --leaks` no longer reports a leak on a bare redaction marker standing as the username component of a generic home prefix (TASK-56). The redactor replaces a matched home prefix with `~` and keeps the surrounding bytes, so a generic prefix whose own username component is empty survives with the marker behind it: with `$HOME=/Users/alice`, the evidence `/Users//Users/alice/x` stores as `/Users/~/x` and `-Users-/Users/alice/x` stores as `-Users-~/x`. The scanner read that `~` as a first component and flagged a line holding no home bytes. A generic home prefix whose username component is exactly `~` is now not a leak, in both the slash and dash-encoded forms; a component that merely starts with `~`, such as `/Users/~abc`, is a real directory name and still reports. Detection of the exact current home, its dash-encoded form, and every other generic-prefix component is unchanged, and `doctor` stays diagnose-only for `--leaks`. Because `doctor --leaks` is a CI gate, a log holding one of these two shapes and no other finding now passes at exit 0 where it previously failed at exit 1; no stored bytes change. One residual of the same class survives, tracked as TASK-59, the entropy-marker residual: the secret pass runs after home rewriting and can replace the bytes right behind an emitted marker, so with `$HOME=/Users/alice` the evidence `/Users//Users/alice/<32-char high-entropy token>` stores as `/Users/~<redacted>`, whose component is not the bare marker and still flags. Design doc r39.
...
- `resolve --dry-run` now always warns that no resolve event was appended, whatever the mix of open and already-resolved requested IDs (TASK-67). The dry-run warning sat as the final arm of an exclusive `if`/`else if`/`else if` chain behind the two already-resolved arms, so either one consumed it: a mixed dry run (one open ID and one already-resolved ID) warned only `["already resolved: 1 ID (bl_...)"]` — identical to the mixed real run that actually appends — and a dry run where every requested ID was already resolved warned only `["already resolved"]`, never saying the run was a preview. Nothing was ever wrong with the stored state or the exit code: `data.changed` and `records[]` were correct in every case, and no bytes were appended by either dry run. The warning *set* was incomplete, so a consumer reading `meta.warnings` alone could not tell a preview from an applied batch. The dry-run warning is now an independent `if` appended after the already-resolved chain, so it fires whenever `--dry-run` is set, regardless of how many requested IDs are already resolved; already-resolved wording, counts, and sort order are unchanged, and every real run's warnings are unchanged. Warning order is now discovery warnings, then the already-resolved warning if any, then `"dry run; no resolve event appended"`: the mixed dry run above now warns `["already resolved: 1 ID (bl_...)", "dry run; no resolve event appended"]`, and the all-already-resolved dry run now warns `["already resolved", "dry run; no resolve event appended"]`. Compatibility: this only adds to the warning array, it does not reword an existing entry, but a caller asserting exact equality on `meta.warnings` for a mixed or all-already-resolved dry run now sees two entries where it previously saw one.

- `add --stderr-file` no longer lets its 4096-byte cap manufacture a home-path boundary the redactor never judged (TASK-68). This one **failed unsafe**, unlike TASK-63: real home bytes reached an append-only log, `leak` is not a fixable finding (`doctor --fix` repairs only `torn_line`, `malformed`, and `conflict_marker`), and there was no repair path short of `archive` or a hand edit — blotter failed its own r22 CI gate on a file blotter wrote, the r30 defect class, with the writer at fault rather than the gate. `redact_and_truncate` redacts before it truncates (r25), so the cap runs after the only pass that judges a home-path boundary, and r42's exact-home rule accepts end of input as an end boundary: cutting a stderr excerpt to 4096 bytes can remove the very byte that declined a match — r42's own deliberately-kept `x/Users/alice2` class, where the following byte blocks the exact match and the preceding byte blocks the generic prefix — and promote it into a leak once that blocking byte is gone. With `$HOME=/Users/alice` and a stderr file holding `('z' * (4096 - len(HOME))) + HOME + 'XXXX'`, stored `evidence.stderr` used to be 4096 bytes ending `…zzz/Users/alice` (or the dash-encoded `…zzz-Users-alice`), and `doctor --leaks` exited 1 on either spelling; the same bytes short enough to skip the cap stored verbatim and passed at exit 0, which stays r42's documented behavior and is now the regression's control.

  The fix moves the write side, not the boundary rule: narrowing r42's end-of-input acceptance would stop a bare `/Users/alice` — the commonest input the redactor sees — from redacting at all, is not expressible in a predicate that sees only a string and not its provenance, and would blind `doctor --leaks` to a line that genuinely ends in the literal home, the r30 direction. `redact_and_truncate` (`src/commands/add.rs`) now re-runs the home pass over exactly the bytes about to be stored, after the r43 marker backtrack, but only when the cap actually cut: `rewrite_home_paths` is not idempotent — its own `~` can fill a generic component that was empty when the first pass judged it, so a second pass collapses r42's own worked example (`/Users//Users/alice-Users-bob-x` → `/Users/~-Users-bob-x` after one pass) one step further to `~` — and running it unconditionally would spell the same bytes one way through `--stderr-file` and another through `--evidence`, the divergent-lane spelling r30 already refused. New order of operations: home pass, secret pass, cap, r43's marker backtrack, home pass again, the last step only when the cap cut. With the fix, the same repro now stores 4085 bytes ending `…zzz~` on both spellings and `doctor --leaks` exits 0.

  One shipped test's expectation moves as a result, deliberately: `stderr_truncation_never_splits_the_secret_marker`'s stored value collapses one further home occurrence the backtrack had left standing against a marker, from `…x /Users/~` to `…x ~`. The backtrack's own behavior is unchanged — the cut still lands at the marker span's start and no partial marker is ever stored — and a new independent test, `stderr_truncation_backtrack_survives_the_home_pass`, pins that on a shape with no `/` or `-` near the cut, so the second home pass cannot touch it.

  Compatibility: no record ID moves, because `compute_id` hashes the version literal, kind, `ts`, `agent`, `text`, severity, and tags, never evidence (r34; design doc: "Evidence is not part of the ID"). Stored history is never rewritten — a log already holding a cap-promoted home still fails `doctor --leaks` with no `--fix` path; the remedy is `archive` or a manual edit of the affected line, same as TASK-62/63. The 4096-byte ceiling still holds — a pass can only replace bytes with the shorter marker, never lengthen — but r43's "at most nine further bytes are dropped, only at that boundary" no longer bounds the stored length on its own; the cap bounds it, and nothing else does. Design doc r46.
...
- Tooling (TASK-57): `scripts/dev/generate-scale-fixtures.py` gains 100k and 300k fixture sizes alongside the existing 1k and 10k, selectable with `--fixtures` (default unchanged: 1k, 10k); `scripts/dev/bench-baseline.sh` gains matching `--fixtures`/`--commands` selection flags so the larger sizes used ad hoc for the TASK-45 and verify recurrence-scan measurements (`docs/research/2026-08-18-scale-baselines.md`) can now be generated and benchmarked through the committed scripts instead of a scratch wrapper. Default invocation with no new flags is unchanged.
