...
- Tests: `tests/cli.rs` is split into `tests/cli/` subject modules under one integration-test binary (TASK-28.2). The 11,312-line single file was one append target, so parallel branches conflicted at its tail on every merge — the top cluster in `blotter triage`. The move is mechanical: 270 tests before and after, no test removed, merged, renamed, or reworded. `AGENTS.md` carries the module map and the placement rule, and `every_test_module_file_is_declared_in_main` fails if a module file is not declared in `main.rs`, which cargo would otherwise leave uncompiled with its tests silently never running.
